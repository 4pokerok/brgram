...
- исключения дублирования по `ref_place.is_except` (если обе станции отмечены флагом, окно не рвётся)
- для МЦД (ЦППК/МТППК) учитывается `ref_place.ln_name2`: допускается одна пересадка между линиями, обратный возврат на исходную линию в том же окне запрещён
...
## Тарифный профиль (UPD)

- Москва: `50 ₽` (`5000` коп)
- Область (досписание на выходе): `20 ₽` (`2000` коп)
- Вход сразу в области: `70 ₽` (`7000` коп)
...
- worker бесконечно ретраит подключение (не падает после первых таймаутов)
